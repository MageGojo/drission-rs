...
// 提示文本(要点哪些字)改用库的元素 API(`tab.ele(...).text()`)读,不再内联 JS,见下文调用点。
...
async fn main() -> drission::Result<()> {
    ...
    for i in 0..n {
        ...
        // 提示文字:用库的元素 API 读(`tab.ele(...).text()` = innerText),不再内联 JS。
        let tips = if let Ok(e) = tab
            .ele("css:.yidun_tips__answer,.yidun_tips,.yidun_tips__text")
            .await
        {
            e.text().await.unwrap_or_default()
        } else {
            String::new()
        };
