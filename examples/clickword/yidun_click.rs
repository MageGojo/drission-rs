...
// 提示文字 / 结果提示改用库的元素 API(`tab.ele(...).text()`)直接读,不再内联 JS,见下文调用点。
...
async fn main() -> drission::Result<()> {
    ...
    for attempt in 1..=tries {
        ...
        let targets: Vec<String> = front
            ...
            .map(|c| c.to_string())
            .collect();
        // DOM 提示文字:用库的元素 API 读(`tab.ele(...).text()` = innerText),不再内联 JS。
        let tips = if let Ok(el) = tab
            .ele("css:.yidun_tips__answer, .yidun_tips, .yidun_tips__text")
            .await
        {
        ...
        if want_click && mv.is_valid() {
            ...
            let result_tip = if let Ok(el) = tab.ele("css:.yidun_tips__text").await {
                el.text().await.unwrap_or_default()
            } else {
                String::new()
            };
