...
async fn main() -> drission::Result<()> {
    ...
    tab.ele("#finp")
        ...
        .await?;
    let finp_val = tab.ele("#finp").await?.value().await?;
    let submitted = tab.run_js("window.__submitted===true").await?;
    let inputkeys_ok = finp_val == "abc" && submitted.as_bool() == Some(true);
    ...
    let after_bs = tab.ele("#inp").await?.value().await?;
    let key_ok = after_bs == "hell";
